...
class SphCandidate : public SphObject
{
  public:
    ...
    SphCandidate(real_t viscosity) : viscosity_alpha(viscosity) {}
...
class SphFluid : public SphObject
{
  ...
  public:
    SphFluid(real_t restDensity, real_t viscosity, real_t tension) : restDensity_rho0(restDensity), viscosity_alpha(viscosity), surfaceTension_gamma(tension) {}
...
class SphBase
{
  ...
  public:
    ...
    Fluid &addFluid(Fluid &f)
    {
        m_Fluids.push_back(f);
        return m_Fluids.back();
    }
