...
public:
	...
	virtual void setupScene()
	{
		...
		while (fin0.getline(line, sizeof(line)))
		{
			...
			t += 0.04;
			i++;
			//std::cout << "这是第？行" << lineflag << std::endl;
		}
		fin0.clear();
		fin0.close();
		...
		//m_TH.vis = vis;
		if (vec_t::dim == 3) {
			//设置一些系数//设置一些系数//设置一些系数//设置一些系数//设置一些系数//设置一些系数//设置一些系数//设置一些系数
			...
			//设置仿真空间大小//设置仿真空间大小//设置仿真空间大小//设置仿真空间大小//设置仿真空间大小//设置仿真空间大小
			...
			//盒子的大小以及添加盒子盒子的大小以及添加盒子盒子的大小以及添加盒子盒子的大小以及添加盒子盒子的大小以及添加盒子
			rb[0] = 13.0f;//长度x
			...
			//bp0.color[3] = 1;
			bp0.position = bp0.velocity = bp0.force = vec_t::O;
			//添加三根柱子添加三根柱子添加三根柱子添加三根柱子添加三根柱子添加三根柱子添加三根柱子添加三根柱子添加三根柱子
			rb[0] = 0.5f;//长度
			rb[1] = 0.5f;//高度
			rb[2] = 0.5f;//宽度
			//第一个是x，第二个是z，第三个是y
			...
			//addFluidCuboid(true, 0, vec_t(-7.35f, -2.0f, -2.44f), vec_t(-3.55f, 5.5f, 2.44f), fp0, 1000, vis, 1);//比Bender 3倍
